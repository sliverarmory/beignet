...
func main() {
	...
	targets := []struct {
	...
	}{
		{
			...
			goSuffix:  "DarwinArm64",
		},
		{
			id:        "darwin_amd64",
			zigTarget: "x86_64-macos",
			goSuffix:  "DarwinAMD64",
		},
	}

	for _, target := range targets {
		loaderBin := filepath.Join(tmpDir, "beignet_loader_"+target.id)
		...
		if *printSize {
			fmt.Fprintf(os.Stdout, "beignet_loader %s image: %d bytes, entry offset: 0x%x\n", target.id, len(image), entryOffset)
		}

		outBin := filepath.Join(*outDir, "stager_"+target.id+".bin")
		if err := os.WriteFile(outBin, image, 0o644); err != nil {
			fatal(err)
		}

		outGo := filepath.Join(*outDir, "stager_"+target.id+"_gen.go")
		if err := os.WriteFile(outGo, []byte(renderGo(target.id, target.goSuffix, entryOffset)), 0o644); err != nil {
			fatal(err)
		}
	}
}
...
func renderGo(id, goSuffix string, entryOffset uint64) string {
	...
	fmt.Fprintln(&buf)
	fmt.Fprintf(&buf, "//go:embed stager_%s.bin\n", id)
	fmt.Fprintf(&buf, "var image%s []byte\n", goSuffix)
	fmt.Fprintln(&buf)
	fmt.Fprintf(&buf, "const entryOffset%s = 0x%x\n", goSuffix, entryOffset)
	fmt.Fprintln(&buf)
	fmt.Fprintf(&buf, "func loaderText%s() ([]byte, uint64) {\n", goSuffix)
	fmt.Fprintf(&buf, "\treturn image%s, entryOffset%s\n", goSuffix, goSuffix)
	fmt.Fprintln(&buf, "}")
	return buf.String()
}
